...
# 步骤2（替代）：ALS 回归（含空间交换项，推荐）
# 需要预先计算空间权重矩阵，或使用已有的缓存
python -m src als \
    --cachedir ./cache \
    --districts data/districts.gpkg \
    --x-f popmean \
...
X_F, X_S, f_Ta_coeffs, y_residual, era5_Ta_mean, Ts_mean = model.prepare_regression_data(
    ...
    landsat_lst_column='landsat_lst_mean',  # 地表温度，用于初始化
...
ALS 回归模式，从能量平衡系数求解各街区气温。

**必需参数**：
- `--cachedir`: 缓存目录路径（physics模块输出）
- `--districts`: 街区矢量数据路径 (.gpkg)
- `-o, --output`: 输出文件前缀（将生成 .gpkg, _Ta.tif, _coefficients.csv）

**可选参数**：
- `--voronoi`: Voronoi图路径（用于栅格化，可选）
- `--district-id`: 街区ID字段名 (默认: district_id)
- `--x-f`: 人为热Q_F特征列名（连续变量），逗号分隔
- `--x-s`: 储热ΔQ_Sb特征列名（连续变量），逗号分隔
- `--x-c`: 分类特征列名（将进行one-hot编码），逗号分隔
- `--spatial-weights`: 空间权重矩阵缓存目录路径，启用水平交换项 ΔQ_A = λ·(Ta - W·Ta)
- `--max-iter`: ALS最大迭代次数 (默认: 500)
- `--tol`: 收敛容差 (默认: 1e-4)
- `--ridge-alpha`: 岭回归正则化参数（0表示普通最小二乘，推荐值1e3-1e6）
- `--quiet`: 静默模式

**空间交换项说明**：

`--spatial-weights` 参数用于加入水平热交换项 ΔQ_A，解决局部冷岛（如水体）气温计算过低的问题。

空间权重矩阵可以通过以下方式预先计算：
```python
from src.analysis import SpatialWeightMatrix
import geopandas as gpd
...
spatial_weights = SpatialWeightMatrix(
    ...
    cache_dir='./cache'  # 保存到此目录
)
```

如果空间权重矩阵只覆盖部分街区（例如只计算了中心城区），ALS 回归会自动：
1. 识别覆盖范围内的街区
2. 只对这些街区应用空间交换项
3. 其他街区正常计算（不受空间效应影响）
...
$$T_a^{(0)} = T_s + (\overline{T_{a,\text{ERA5}}} - \overline{T_s})$$

这种初始化保留了地表温度的空间异质性，使空间交换项 `λ·(Ta - W·Ta)` 在第一次迭代就能产生作用。

**注意事项**：
- 空间交换项只对覆盖范围内的街区有效
- 如果要让水体等局部冷岛受益于空间效应，需确保水体在覆盖范围内且其邻居包含高温区域
- λ 值会在迭代中通过回归自动估计
...
**如果使用了空间交换项（--spatial-weights），还会输出**：
- `lambda_coeff`: 空间交换系数 λ
- `spatial_lag`: 邻域平均气温 W·Ta (K)
- `spatial_coverage`: 空间权重覆盖率

**解读**：
- `Ta_optimized` 是每个街区的估算近地表气温
- `coeff_F_*` 表示人为热对气温的影响
- `coeff_S_*` 表示储热对气温的影响
- `lambda_coeff` 表示水平热交换强度：
  - λ > 0：气温趋向于邻域平均（热量扩散），符合物理预期
  - λ ≈ 5-15：典型城市环境（较强的空间交换）
  - λ 值越大，邻域影响越强
- `spatial_lag` 表示该街区周围邻居的加权平均气温
...
**问题**：使用 `--spatial-weights` 后，气温结果与不使用时相同

**可能原因**：
1. 空间权重矩阵只覆盖同质区域（都是同类型建成区）
2. 关注的区域（如水体）不在覆盖范围内
3. 覆盖范围内的样本物理特性相似，Ta 收敛后 Ta - W·Ta ≈ 0

**诊断方法**：
```python
# 检查 λ 值
# 如果 λ ≈ 0，说明空间效应很弱
# 如果 λ ≈ 5-15，说明空间效应显著但可能不改变稳态解
print(f"空间交换系数 λ = {results['lambda_coeff']}")

# 检查覆盖范围内的 LCZ 分布
covered_mask = results['spatial_mask']
covered_lcz = districts_gdf.loc[covered_mask, 'LCZ'].value_counts()
print(covered_lcz)
# 如果只有 LCZ 1-6（建成区），没有水体/公园，空间效应会很弱
```

**解决方案**：
1. 扩展空间权重矩阵覆盖范围，包含水体、公园等异质区域
2. 确保关注区域（水体）及其邻居都在覆盖范围内
3. 如果需要对水体单独处理，考虑调整水体的物理参数（储热系数、粗糙度等）
