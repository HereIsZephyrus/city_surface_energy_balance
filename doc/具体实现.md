...
results = model.fit(
    ...
    era5_Ta_mean=era5_Ta_mean,
    Ts_mean=Ts_mean,
    spatial_weights=W,        # 空间权重矩阵（稀疏矩阵）
    spatial_mask=mask,        # 覆盖范围掩码（布尔数组）
    max_iter=100,
    tol=1e-4
)
# 结果额外包含:
# - lambda_coeff: 空间交换系数 λ
# - spatial_lag: 邻域平均气温 W·Ta
```

**初始化策略**：

当提供 `Ts_mean` 和 `era5_Ta_mean` 时，使用基于地表温度的初始化：
$$T_a^{(0)} = T_s + (\overline{T_{a,\text{ERA5}}} - \overline{T_s})$$

这样初始化保留了地表温度的空间异质性，使空间交换项在第一次迭代就能发挥作用。
...
**位置**：`src/regression/als_regression.py`

**功能**：
1. 初始化 Ta（使用 Ts + 偏移量）
2. 固定 Ta，估计线性系数（α, β）和空间交换系数（λ）
3. 固定系数，优化每个街区的 Ta（考虑空间交换）
4. 迭代直到收敛

**关键代码**：
```python
# 初始化 Ta（基于地表温度）
Ta = Ts_mean + (era5_Ta_global_mean - Ts_global_mean)

# 迭代
for iteration in range(max_iter):
    # 步骤1：计算空间滞后项
    W_Ta = spatial_weights @ Ta[spatial_mask]  # 邻域平均气温
    X_spatial = Ta[spatial_mask] - W_Ta        # 空间特征
    
    # 步骤2：固定Ta，估计线性系数和λ
    y = -f_Ta_coeff2 * Ta**2 - f_Ta_coeff1 * Ta - residual
    X = [X_F, X_S, X_spatial]  # 特征矩阵（含空间项）
    coeffs = ridge_regression(X, y, alpha)
    lambda_coeff = coeffs[-1]  # 空间交换系数
    
    # 步骤3：固定系数，优化Ta
    for k in range(n_districts):
        if is_covered[k]:
            # 含空间交换的能量方程：
            # f_Ta_coeff2_k * Ta² + (f_Ta_coeff1_k + λ) * Ta + 
            #   (residual_k + X_k @ α_β - λ * W_Ta_k) = 0
            b = f_Ta_coeff1_k + lambda_coeff
            c = residual_k + linear_pred_k - lambda_coeff * W_Ta_k
            Ta_k = solve_equation(a, b, c)
        else:
            # 不在覆盖范围，无空间效应
            Ta_k = solve_equation(a, f_Ta_coeff1_k, residual_k + linear_pred_k)
...
**物理意义**：
- 当 W_Ta_k > Ta_k（邻域更热），λ × W_Ta_k 使分子增大，Ta 升高（热量流入）
- 当 W_Ta_k < Ta_k（邻域更冷），λ × W_Ta_k 使分子减小，Ta 降低（热量流出）
...
`src/als.py` 中的 `load_spatial_weights()` 函数支持从缓存目录加载预计算的空间权重矩阵：

```python
from src.als import load_spatial_weights

# 从缓存目录加载
W = load_spatial_weights('./cache', verbose=True)

# 支持的格式：
# - spatial_weights.npz: scipy.sparse 矩阵
# - spatial_weights_metadata.pkl: 包含 id 映射和覆盖范围信息

# 部分覆盖处理：
# 如果矩阵只覆盖部分街区（如只有中心城区），
# ALS 回归会自动识别并只对覆盖范围内的街区应用空间效应
```
