...
if __name__ == "__main__":
    ...
    # 示例2: 不同密度场景对比
    print("\n" + "=" * 70)
    print("示例2: 不同建筑密度场景对比")
    print("-" * 70)
    
    scenarios = [
        ("低密度住宅区", np.array([8.0, 9.0, 8.5]), np.array([80.0, 90.0, 85.0]), 1000.0),
        ("中密度住宅区", np.array([12.0, 15.0, 14.0, 13.0]), np.array([120.0, 150.0, 140.0, 130.0]), 1000.0),
        ("高密度商业区", np.array([25.0, 30.0, 28.0, 32.0, 27.0]), np.array([200.0, 250.0, 220.0, 240.0, 210.0]), 1000.0),
    ]
    
    for name, h, a, pa in scenarios:
        bd = BuildingData(heights=h, footprint_areas=a, plot_area=pa)
        p = calculate_roughness_parameters(bd, calculate_porosity=False)
        print(f"\n{name}:")
        print(f"  平均高度: {p.mean_height:.2f} m")
        print(f"  建筑密度: {p.lambda_p:.2%}")
        print(f"  z_d: {p.zero_plane_displacement:.2f} m")
        print(f"  z_0: {p.roughness_length:.4f} m")
