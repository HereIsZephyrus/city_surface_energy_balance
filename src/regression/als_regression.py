...
class ALSRegression:
    ...
    def prepare_regression_data(
        ...
        返回:
            (X_F, X_S, f_Ta_coeff1, f_Ta_coeff2, y_residual, era5_Ta_mean)
        ...
        return X_F, X_S, f_Ta_coeff1, f_Ta_coeff2, y_residual, era5_Ta_mean
    ...
    def fit(
        ...
        for iteration in range(max_iter):
            ...
            # Step 2: 固定α, β，优化Ta
            # 求解能量平衡方程: f(Ta) + linear_pred + y_residual = 0
            # 即 coeff2×Ta² + coeff1×Ta + (linear_pred + y_residual) = 0
            ...
            for i in range(n_districts):
                ...
                if f_Ta_coeff2 is not None:
                    # 二次方程: a×Ta² + b×Ta + c = 0
                    # 其中 c = linear_pred[i] + y_residual[i]
                    a = f_Ta_coeff2[i]
                    b = f_Ta_coeff1[i]
                    c = linear_pred[i] + y_residual[i]
                    ...
                    if discriminant >= 0 and abs(a) > 1e-12:
                        ...
                        if candidates:
                            ...
                            Ta_new[i] = self._optimize_Ta_single(
                                Ta[i], a, b, linear_pred[i], y_residual[i]
                            )
                    else:
                        # 判别式为负或a太小，使用优化
                        Ta_new[i] = self._optimize_Ta_single(
